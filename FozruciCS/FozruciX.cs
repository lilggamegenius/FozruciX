...
namespace FozruciCS {
    ...
    public class FozruciX : ListenerAdapter{
        ...
        private static readonly int JokeCommands = 0;
        private static readonly int ArrayOffsetSet = 1;
        private static readonly int CleverBotInt = 2;
        private static readonly int PandoraBotInt = 3;
        private static readonly int JabberBotInt = 4;
        private static readonly int NickInUse = 5;
        private static readonly int Color = 6;
        private static readonly int RespondToPms = 7;
        private static readonly int DataLoaded = 8;
        private static readonly int CheckLinks = 9;
        ...
        private static BitVector32 _bools = new BitVector32(0); // true, false, null, null, null, false, true, true, false, false
        ...
        private static void SendMessage( GenericMessageEvent @event,  string msgToSend, bool addNick = true, bool splitMessage = false) {
        ...
        private static void AddCooldown(User user) {
            AddCooldown(user, _defaultCoolDownTime);
        }
        ...
        public static string GetArg(string[] args, int index) {
            return GetArg(args, index, _arrayOffset);
        }
        ...
        private static void Log(Event @event, bool botTalking) {
        ...
        private static void Log(Event @event, string messageOverride = null, bool botTalking = false) {
            ...
            if (quitEvent != null) {
                ...
                if (discordQuitEvent != null) {
                    channel.AddRange(discordQuitEvent.getLeaveEvent().getGuild().getTextChannels().toList<TextChannel>().Select(chanName=> "#" +((MessageChannel)chanName).getName()));
        ...
        private static string GetSeverName(Event @event, bool trimAddress = false) {
            ...
            if (messageEvent != null) {
                network = messageEvent.getDiscordEvent().getGuild().getName();
            }
            return network;
        }
        ...
        public static void SendError( MessageEvent @event,  System.Exception e) {
            ...
            var cause = "";
            string from;
            if (_bools[Color] && discordFormatting.isEmpty()) {
                color = Colors.RED;
            }
            if (e.getCause() != null) {
        ...
        private bool CheckCooldown(GenericUserEvent @event) {
            if (@event.getUser() == null ||
                !_commandCooldown.ContainsKey(@event.getUser())) return false;
        ...
        private void SendPrivateMessage( GenericEvent @event,  string userToSendTo,  string msgToSend, bool removeNewLines = true) {
        ...
        private void MakeDebug() {
            if ((_debug == null) || (_debug.ConnectEvent == null)) {
                return;
            }
        ...
        private void MakeDiscord() {
            if (_discord == null && Network != Network.Discord) {
                _discord = DiscordAdapter.makeDiscord(_bot);
            }
        }
        ...
        public override void onConnect( ConnectEvent @event) {
            ...
            if (@event is DiscordConnectEvent) {
                _currentUser = new DiscordUser(new DiscordUserHostmask(_bot, @event.getBot().getUserBot().getHostmask()), DiscordAdapter.getJda().getSelfUser(), null);
            } else {
                _currentUser = @event.getBot().getUserBot();
            }
        ...
        private static void SetArrayOffset( string prefix) {
            ...
            if ((prefix.length() > 1) && !prefix.endsWith(".")) {
                _arrayOffset = StringUtils.countMatches(prefix, " ");
            } else {
                _arrayOffset = 0;
            }
        ...
        private void SetArrayOffset() {
            SetArrayOffset(_prefix);
        }


        private static string[] FormatstringArgs( string[] arg) {
            return TrimFrontOfArray(arg, 1 + _arrayOffset);
        }
        ...
        public override void onMessage( MessageEvent @event) {
        ...
        public void OnMessage(MessageEvent @event, bool log) {
            ...
            if ((Network == Network.Normal) && _bools[NickInUse]) {
                if (!_bot.getNick().equalsIgnoreCase(_bot.getConfiguration().getName())) {
                    ...
                    new Thread(() => {
                        ...
                        _bot.sendRaw().rawLineNow("ns recover " + _bot.getConfiguration().getName() + " " + CryptoUtil.decrypt(FozConfig.Password));
                        _bot.sendRaw().rawLineNow("ns ghost " + _bot.getConfiguration().getName() + " " + CryptoUtil.decrypt(FozConfig.Password));
            ...
            try {
                if (!(@event.getMessage().startsWith(_prefix) && @event.getMessage().startsWith("."))) {
                    if (LilGUtil.endsWithAny(@event.getMessage(), ".", "?", "!")) {
            ...
            var server = Network == Network.Discord ? ((DiscordMessageEvent) @event).getDiscordEvent().getGuild().getId() : @event.getBot().getServerHostname();
    ...
    // url checker - Checks if string contains a url and parses
            try {
                ...
                Logger.Trace(GetSeverName(@event, true) + ": containsServer: " + containsServer + " containsChannel: " + containsChannel + " channelContains: " + channelContains + " " + _allowedCommands);
                ...
                var uriFinderPattern = Pattern.compile("\\(*https?://[" + validChars + validNonTerminal + "]*[" + validChars + "]", Pattern.CASE_INSENSITIVE);
                ...
                try {
                    ...
                    if ((DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond) > _lastLinkTime) {
                        _lastLinkTitle = "";
                    }
                    if (title.isEmpty()) {
            ...
            // !setGuildChan - Sets what channel to announce joins an quits in
                ...
                if (GetArg(arg, 1) != null) {
                    if (GetArg(arg, 1).toLowerCase().startsWith("rem")) {
                        ...
                        var channels = ((DiscordMessageEvent) @event).getDiscordEvent().getGuild().getTextChannels().toList<TextChannel>();
                                ...
                                if (mentioned.Count != 0) {
                                    foreach(var mentionedUser in mentioned) {
                                        ...
                                        if (!mentionedUser.hasPrivateChannel()) {
                                            mentionedUser.openPrivateChannel().complete();
                                        } 
                                        sendMsg.Start();
                                        if (isBan) {
                                        ...
                                        if (DiscordData.channelRoleMap.ContainsKey(mChannel)) {
                                            ...
                                            if (roleArgs.Count != 0)
                                                foreach(var role in guildRoleList) {
                                                    foreach(var roleArg in roleArgs) {
                                                        if(!roleArg.equalsIgnoreCase(role.getName())) continue;
                                                        overRide = mChannel.getPermissionOverride(role) ?? mChannel.createPermissionOverride(role).complete(false) as PermissionOverride;
                                                        if (whiteListMode) {
                                                            overRide.getManager().grant(Permission.MESSAGE_WRITE).queue();
                                                        } else {
                                                            overRide.getManager().deny(Permission.MESSAGE_WRITE).queue();
    ...
    // !muteServer - mutes entire server
                ...
                if (GetArg(arg, 1) != null) {
                    if (GetArg(arg, 1).equalsIgnoreCase("add")) {
                        _mutedServerList.Add(GetArg(arg, 2));
                    } else if (GetArg(arg, 1).equalsIgnoreCase("del")) {
                        _mutedServerList.Remove(GetArg(arg, 2));
                    }
                } else {
                    _mutedServerList.Add(Network == Network.Discord ? ((DiscordMessageEvent) @event).getDiscordEvent().getGuild().getId() : @event.getBot().getServerHostname());
                }
            }
                            ...
                            for (byte i = 2; GetArg(commandArg, i) != null; i++) {
                                ...
                                switch(mode){
                                    ...
                                    case 1:
                                        ...
                                        break;
                                    default: SendMessage(@event, "The command " + command + " is " + ((allowedCommands[chan]).Contains(command) ? "" : "not ") + "Banned from " + chan);
                                        break;
                                }
                            }
                        } else if (GetArg(commandArg, 1) != null) {
                            SendMessage(@event, allowedCommands[GetArg(commandArg, 1)].toString());
                        } else {
    ...
    // !Commands - lists commands that can be used
                    ...
                    if (GetArg(arg, 1) == null) {
                        SendNotice(@event, @event.getUser().getNick(), "List of Commands so far. for more info on these Commands do " + _prefix + "Commands. Commands with \"Joke: \" are joke Commands that can be disabled");
    ...
    // !serverHostName - Gets the Server Host Name
                    ...
                    if (CheckPerm(@event.getUser(), 9001)) {
                        SendMessage(@event, _bot.getServerHostname(), false);
                    } else {
                        PermErrorchn(@event);
                    }
                }
    ...
    // !Connect - joins a channel
                    ...
                    if (CheckPerm(@event.getUser(), 9001)) {
                        _bot.send().joinChannel(GetArg(arg, 1));
                    } else {
                        PermErrorchn(@event);
                    }
                }
...
/*                else if (commandChecker(@event, arg, "setDebugLevel")) {
    ...
    // !markov - makes Markov chains
                    ...
                    if (MarkovChain == null) {
                        MarkovChain = new Dictionary<string, List<string>>();
                    }
                    var loop = true;
                    var newPhrase = "";
                    try {
    ...
    // !CheckLink - checks links, duh
                    ...
                    try {
                        var doc = Jsoup.connect(GetArg(arg, 1)).userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2783.4 Safari/537.36").timeout(5000).get();
                        ...
                        if (GetArg(arg, 1).equalsIgnoreCase("set")) {
                            try {
                                if (_authedUser.Contains(GetArg(arg, 2))) {
                                    try {
                                        _authedUserLevel[_authedUser.IndexOf(GetArg(arg, 2))] = Convert.ToInt32(GetArg(arg, 3));
                                    } catch (Exception e) {
                                        SendError(@event, e);
                                    }
                            ...
                            SendMessage(@event, _authedUser.toString());
                        } else {
                            var place = -1;
                            try {
                                for (var i = 0; _authedUser.Count >= i; i++) {
                                    if (((string) _authedUser[i]).equalsIgnoreCase(GetArg(arg, 1))) {
                                        place = i;
                                    }
                                }
                            ...
                            if (place == -1) {
                                ...
                                SendMessage(@event, "User " + (string) _authedUser[place] + " Has permission level " + (int) _authedUserLevel[place], false);
                            }
                        ...
                        foreach(var pod in queryResult.getPods()) {
                            ...
                            foreach(var subPod in pod.getSubpods()) {
                                foreach(var element in subPod.getContents()) {
                                    ...
                                    var elementResult = ((WAPlainText) element).getText().replace('\uF7D9', '=').replace("\uF74E", "\u001Di\u001D");
    ...
    // !CalcJ - calculate a expression
                    ...
                    parser.addArgument("-v", "--Val").type(typeof(double)).setDefault(-1.0)
                    ...
                    parser.addArgument("-s", "--step").type(typeof(double)).setDefault(1.0)
                            .help("Sets How much to increase x at");
                    parser.addArgument("-a", "--amount").type(typeof(sbyte)).setDefault(3)
                            .help("Sets How many times to increase");
                    parser.addArgument("-p", "--precision").type(typeof(long)).setDefault(64L)
                    ...
                    try {
                        ...
                        if (LilGUtil.containsAny(message, "-v", "-c", "-s", "-a", "--Val", "--char", "--step", "--amount")) {
                            ...
                            while (count <= calcAmount) {
                                ...
                                eval.Add(Evaluator.evaluate(ArgJoiner(ns.getList("expression").toList<string>().ToArray(), 0).toLowerCase(), VariableSet) as Apfloat);
                                x += step;
                                count++;
                            }
                    ...
                    if (CheckPerm(@event.getUser(), 1)) {
                        if (GetArg(arg, 1) != null && GetArg(arg, 1).equalsIgnoreCase("setup")) {
                            ...
                            if (GetArg(arg, 3) != null) {
                                _counterCount = Convert.ToInt32(GetArg(arg, 3));
                            }
                        }
                            ...
                            if (GetArg(arg, 2) != null) {
                                ...
                                if (arg.Length > subCommandNum + _arrayOffset && arg[subCommandNum - 1].equalsIgnoreCase("Example")) {
                                    if (sense.getExamples().size() > 0) {
                                        lookedUpWord = ((IWikiString) sense.getExamples().get(0)).getPlainText();
                                    } else {
                    ...
                    try {
                        if (GetArg(arg, 2) != null) {
                            ...
                            if (GetArg(arg, 1).equalsIgnoreCase("list")) {
                                SendMessage(@event, _fcList.Keys.toString());
                        ...
                        try {
                            if (GetArg(arg, 1).equalsIgnoreCase("set")) {
                                if (_memes.ContainsKey(GetArg(arg, 2).toLowerCase().replace("\u0001", ""))) {
                                    ...
                                    _memes[GetArg(arg, 2).toLowerCase().replace("\u0001", "")] = new Meme(@event.getUser().getNick(), ArgJoiner(arg, 3).replace("\u0001", ""));
                                    SendMessage(@event, "Meme " + GetArg(arg, 2) + " Created as " + ArgJoiner(arg, 3));
                                }
                            } else if (GetArg(arg, 1).equalsIgnoreCase("list")) {
                                SendMessage(@event, _memes.toString().replace("\u0001", ""));
                            } else {
                                if (_memes.ContainsKey(GetArg(arg, 1).toLowerCase())) {
                                    SendMessage(@event, GetArg(arg, 1).replace("\u0001", "") + ": " + ((Meme) _memes[GetArg(arg, 1).toLowerCase()]).getMeme().replace("\u0001", ""), false);
                                } else {
    ...
    // !q - adds to q
                    ...
                    try {
                        if (GetArg(arg, 1).equalsIgnoreCase("del") || GetArg(arg, 1).equalsIgnoreCase("pop") || GetArg(arg, 1).equalsIgnoreCase("rem")) {
                            if (_qList.Remove(@event.getUser().getNick())) {
                            ...
                            if (_qList.Add(@event.getUser().getNick())) {
                            ...
                            SendMessage(@event, "READY?!?!1/1 " + (time != 10 ? "starting in " + time + " sec!!!! " : "") + _qList.toString().replace("[", "").replace("]", ""), false);
                            LilGUtil.pause(time);
                            ...
                            SendMessage(@event, "Current time: " + _qTimer.toString());
                        }
                        AddCooldown(@event.getUser());
    ...
    // !version - gets the version of the bot
                    ...
                    var version = "PircBotX: " + PircBotX.VERSION + ". BotVersion: " + Version + ". Java version: " + global::java.lang.System.getProperty("java.version");
    ...
    // !prefix - Changes the command prefix when it isn't the standard "!"
                    ...
                    if (CheckPerm(@event.getUser(), 9001)) {
                        if (GetArg(arg, 1) != null) {
                            ...
                            SendMessage(@event, "Command variable is \"" + _prefix + "\"");
                        }
                    } else {
    ...
    // !prefix - Changes the command prefix
                    ...
                    if (CheckPerm(@event.getUser(), 9001)) {
                        ...
                        if (_prefix.length() > 1 && !endsWithAny(_prefix, ".", "!", "`", "~", "@", "-", "/", "*", "&", "^", "%", "$", "#", "+", "_", "?", "\\", ";", ":", "|")) {
                            _arrayOffset = 1;
                        } else {
                            _arrayOffset = 0;
                        }
    ...
    // !S1TCG - Create Title card info
                    ...
                    if (GetArg(arg, 1) != null) {
                        try {
                            ...
                            if (title.Count < 7) {
                                foreach(var aTitle in title) {
                                    sendNotice(@event, @event.getUser().getNick(), aTitle);
                                }
                            } else {
                                SendPage(@event, arg, new List<string>(title));
                            }
                            AddCooldown(@event.getUser());
                        } catch (Exception e) {
                            SendError(@event, e);
                    ...
                    try {
                        ...
                        process.WaitForExit();
                            ...
                            if (output.Count > 3) {
                                ...
                                foreach(string anOutput in output) {
    ...
    // !Trans - Translate from 1 language to another
                    ...
                    try {
                        ...
                        if (ns.getBoolean("detect").booleanValue()) {
                            ...
                            Language from;
                            from = ns.getString("from").Equals("detect") ? Detect.execute(textToTrans) : Language.valueOf(ns.getString("from").toUpperCase());
    ...
    // > - runs COMMANDS without closing at the end
                    ...
                    if (CheckPerm(@event.getUser(), 9001)) {
                        if (arg[0].substring(_consolePrefix.length()).equalsIgnoreCase(_consolePrefix + "start")) {
                            _terminal.interrupt();
                            _terminal = new CommandLine(@event, TrimFrontOfArray(arg, 1));
                            _terminal.start();
                            if (!_terminal.isAlive()) {
    ...
    // !SayNotice - Tells the bot to send a notice
                    ...
                    if (CheckPerm(@event.getUser(), 6)) {
                        _bot.sendIRC().notice(GetArg(arg, 1), ArgJoiner(arg, 2));
                    } else {
                        PermErrorchn(@event);
                    }
                }
    ...
    // !SayCTCPCommand - Tells the bot to send a CTCP Command
                    ...
                    if (CheckPerm(@event.getUser(), 9001)) {
                        _bot.sendIRC().ctcpCommand(GetArg(arg, 1), ArgJoiner(arg, 2));
                    } else {
                        PermErrorchn(@event);
                    }
                }
                    ...
                    if (CheckPerm(@event.getUser(), 9001)) {
                        ...
                        try {
                            Thread exit = new Thread(() -> {
                                if (GetArg(arg, 1) != null) {
                                    _manager.stop(ArgJoiner(arg, 1));
                                } else {
                                    _manager.stop("I'm only a year old and have already wasted my entire life.");
                                }
                                try {
                                    LilGUtil.pause(1);
    ...
    // !quitServ - Tells the bot to disconnect from server
                    ...
                    if (CheckPerm(@event.getUser(), 9001)) {
                        ...
                        if (GetArg(arg, 1) != null) {
                            _bot.sendIRC().quitServer(ArgJoiner(arg, 1));
                        } else {
                            _bot.sendIRC().quitServer("I'm only a year old and have already wasted my entire life.");
                        }
    ...
    // !getUserLevels - gets the user levels of the user
                    ...
                    try {
                        List<UserLevel> userLevels = Lists.newList(@event.getUser().getUserLevels(@event.getChannel()).iterator());
    ...
    // !getMem - Gets various info about memory
                    ...
                    var send = "Current memory usage: " + LilGUtil.formatFileSize(runtime.totalMemory() - runtime.freeMemory()) + "/" + LilGUtil.formatFileSize(runtime.totalMemory()) + ". Total memory that can be used: " + LilGUtil.formatFileSize(runtime.maxMemory()) + ".  Active Threads: " + Thread.activeCount() + "/" + ManagementFactory.getThreadMXBean().getThreadCount() + ".  Available Processors: " + runtime.availableProcessors();
    ...
    // !jToggle - toggle joke COMMANDS
                    ...
                    if (GetArg(arg, 1).equalsIgnoreCase("toggle")) {
                        if (CheckPerm(@event.getUser(), 2)) {
                            _bools.flip(JokeCommands);
                            if (_bools.get(JokeCommands)) {
    ...
    // !sudo/make me a sandwich - You should already know this joke
                    ...
                    if (_bools.get(JokeCommands) || CheckPerm(@event.getUser(), 1)) {
    ...
    // !potato - Joke command - say "i am potato" in Japanese
                    ...
                    if (_bools.get(JokeCommands) || CheckPerm(@event.getUser(), 1)) {
                        var bytes = "わたしわポタトデス".getBytes(Charset.forName("UTF-8"));
                        var v = new string(bytes, Charset.forName("UTF-8"));
                        SendMessage(@event, v);
                    } else
                        SendMessage(@event, " Sorry, Joke COMMANDS are disabled");
    ...
    // !WhatIs? - Joke command -
                    ...
                    if (_bools.get(JokeCommands) || CheckPerm(@event.getUser(), 1)) {
                        var num = LilGUtil.randInt(0, Dictionary.length - 1);
    ...
    // !rip - Joke command - never forgetti the spaghetti
                    ...
                    if (_bools.get(JokeCommands) || CheckPerm(@event.getUser(), 1)) {
                        ...
                        } else if (GetArg(arg, 1).equalsIgnoreCase(_bot.getNick())) {
    ...
    // !rejoin - Rejoins all channels
            else if (pm.getMessage().equalsIgnoreCase(_prefix + "rejoin")) {
                ImmutableList<string> autoChannels = _bot.getConfiguration().getAutoJoinChannels().keySet().asList();
                for (var i = 0; i < autoChannels.Count; i++) {
                    _bot.send().joinChannel(autoChannels[i]);
                }
            }
    ...
    // !login - Sets the authed named to the new name ...if the password is right
                ...
                if (CryptoUtil.encrypt(GetArg(arg, 1)).Equals(FozConfig.Password)) {
                    ...
                    if (Network == Network.Discord) {
                        _currentUser = new DiscordUser(pm.getUserHostmask(), ((DiscordPrivateMessageEvent) pm).getDiscordEvent().getAuthor(), null);
                    }
    ...
    // !part - leaves a channel
                    ...
                    if (Network != Network.Discord) {
                        if (GetArg(arg, 2) != null) {
                            _bot.sendRaw().rawLineNow("part " + GetArg(arg, 1) + " :" + ArgJoiner(arg, 2));
                        } else {
                            _bot.sendRaw().rawLineNow("part " + GetArg(arg, 1));
                        }
    ...
    // !Connect - Tells the bot to connect to specified channel
                    ...
                    if (GetArg(arg, 2) != null) {
                        _bot.sendIRC().joinChannel(GetArg(arg, 1), GetArg(arg, 2));
                    } else {
                        _bot.sendIRC().joinChannel(GetArg(arg, 1));
                    }
    ...
    // !QuitServ - Tells the bot to disconnect from server
            ...
            if (pm.getMessage().startsWith(_prefix) || pm.getMessage().startsWith(_consolePrefix) || pm.getMessage().startsWith(pm.getBot().getNick())) {
                DoCommand(new MessageEvent(_bot, new DiscordChannel(_bot, pm.getUser().getNick()), pm.getUser().getNick(), pm.getUserHostmask(), pm.getUser(), pm.getMessage(), null));
            } else {
                try {
        ...
        public void OnNotice( NoticeEvent @event) {
            ...
            if (!@event.getUser().getNick().equalsIgnoreCase("NickServ") || !@event.getUser().getNick().equalsIgnoreCase("irc.badnik.net")) {
                ...
                if (message.contains("*** Found your hostname") ||
                    ...
                    @event.getUser().send().notice("\u0001AVATAR " + _avatar + "\u0001");
                } else {
                    _bot.sendIRC().notice(_currentUser.getNick(), "Got notice from " + @event.getUser().getNick() + ". Notice was : " + @event.getMessage());
                }
            }
        ...
        public void OnAction( ActionEvent action) {
            //noinspection ConstantConditions
            OnMessage(new MessageEvent(_bot, action.getChannel(), action.getChannelSource(), action.getUserHostmask(), action.getUser(), action.getAction(), null), false);
            Log(action);
        }
        ...
        public void OnQuit( QuitEvent quit) {
            ...
            if (quit is DiscordQuitEvent) {
                ...
                if (channelToMessage != null) {
                    List<TextChannel> channels = ((DiscordQuitEvent) quit).getLeaveEvent().getGuild().getTextChannels().toList<TextChannel>();
                    foreach(TextChannel channel in channels) {
                        if (channel.getId().Equals(channelToMessage)) {
        ...
        private bool CheckPerm(User user, int requiredUserLevel) {
            ...
            if (_authedUser.Contains(user.getNick())) {
                ...
                if (index > -1) {
                    if (_authedUserLevel[index] >= requiredUserLevel) {
                        return true;
                    }
                }
            } else if (user is DiscordUser) {
                if (user.getHostname().Equals(_currentUser.getHostname())) {
                    return true;
                }
                List<Role> roles = ((DiscordUser) user).getGuild().getMember(((DiscordUser) user).getDiscordUser()).getRoles().toList<Role>();
                var highestLevel = 0;
                ...
                while (index > -1) {
                    ...
                    if (matchHostMask(user.getHostmask(), ident)) {
                        return _authedUserLevel[index] >= requiredUserLevel;
                    }
                    index--;
                }
        ...
        private void LoadData(bool writeOnce) {
            if (writeOnce && _noteList == null)
                _noteList = SaveDataStore.getINSTANCE().getNoteList();
    
            if (writeOnce && _authedUser == null)
                _authedUser = SaveDataStore.getINSTANCE().getAuthedUser();
    
            if (writeOnce && _authedUserLevel == null)
                _authedUserLevel = SaveDataStore.getINSTANCE().getAuthedUserLevel();
    
            if (writeOnce && _avatar == null)
                _avatar = SaveDataStore.getINSTANCE().getAvatarLink();
    
            if (writeOnce && _memes == null)
                _memes = SaveDataStore.getINSTANCE().getMemes();
    
            if (writeOnce && _fcList == null)
                _fcList = SaveDataStore.getINSTANCE().getFCList();
    
            if (writeOnce && MarkovChain == null)
                MarkovChain = SaveDataStore.getINSTANCE().getMarkovChain();
    
            if (writeOnce && _allowedCommands == null)
                _allowedCommands = SaveDataStore.getINSTANCE().getAllowedCommands();
    
            if (writeOnce && _checkJoinsAndQuits == null)
                _checkJoinsAndQuits = SaveDataStore.getINSTANCE().getCheckJoinsAndQuits();
    
            if (writeOnce && _mutedServerList == null)
                _mutedServerList = SaveDataStore.getINSTANCE().getMutedServerList();
        ...
        private void CheckNote( Event @event,  string user,  string channel) {
            ...
            try {
                for (var i = 0; i < _noteList.Count; i++) {
                    ...
                    if (channel != null) {
                        try {
                            SendMessage((MessageEvent) @event, user + ": " + _noteList[i].displayMessage());
                        } catch (ClassCastException e) {
                            //noinspection ConstantConditions
                            SendPrivateMessage(((JoinEvent) @event).getUser().getNick(), user + ": " + _noteList[i].displayMessage());
                        }
                    } else {
                        SendNotice(@event, user, _noteList[i].displayMessage());
                    }
        ...
        private bool CommandChecker(GenericMessageEvent @event, string[] args, string command, bool printMsg) {
            ...
            try {
                ...
                if (args[0].startsWith(_prefix)) {
                    if (_prefix.length() > 1 && !_prefix.endsWith(".")) {
                        isCommand = GetArg(args, 0).equalsIgnoreCase(command);
                    } else {
                        isCommand = args[0].equalsIgnoreCase(_prefix + command);
                    }
                ...
                if (isCommand) {
                    Dictionary<string, List<string>> temp = _allowedCommands[GetSeverName((Event) @event, true)];
                    List<string> commands;
                    if (temp != null) {
                        commands = temp[chanName];
                    } else {
                        commands = null;
                    }
                    if (commands != null && commands.Contains(command.toLowerCase())) {
                        ...
                        if (@event is DiscordMessageEvent) /*then*/
        ...
        private void GetHelp(GenericMessageEvent @event, string command) {
            switch (command.toLowerCase()) {
                ...
                case "s1tcg":
                    ...
                    break;
                default:
                    SendNotice(@event, @event.getUser().getNick(), "That either isn't a command, or " + _currentUser.getNick() + " hasn't add that to the help yet.");
                    break;
            }
        }
    
        public void SetAvatar(string avatar) {
            FozruciX._avatar = avatar;
        ...
        private void AddWords( string phrase) {
            if (phrase.startsWith(_prefix) ||
                    phrase.startsWith(_consolePrefix)) {
                return;
            }
            ...
            for (var i = 0; i < words.Length; i++) {
                ...
                if (i == 0) {
                    List<string> startWords = MarkovChain["_start"];
                    startWords.Add(words[i]);
    
                    List<string> suffix = MarkovChain[words[i]];
                    if (suffix == null) {
                    ...
                    List<string> endWords = MarkovChain["_end"];
        ...
        private class Python : Thread {
            ...
            public void Run() {
                try {
                    object temp = _engine.eval(_eval);
                    if (temp != null) {
                        ...
                        if (isNumeric(eval)) {
                            ...
                            SendPage(_event, new[]{"!PY", this._eval, "" + _radix}, new List<>(eval));
                        }
                    }
                } catch (Exception e) {
                    SendError(_event, e);
    ...
    public class JavaScript : Thread {
        ...
        internal JavaScript( MessageEvent @event, string arg, int radix) {
                ...
                try (InputStreamReader algebra = new InputStreamReader(new FileInputStream("algebra.min.js"))) {
                    ...
                    for (string unsafeAttribute : _unsafeAttributes) {
