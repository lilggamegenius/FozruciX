...
public class DiscordAdapter extends ListenerAdapter {
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		if (event.isFromType(ChannelType.PRIVATE)) {
			...
			if (!event.getAuthor().getId().equals(FozConfig.jda.getSelfUser().getId())) {
			...
			try {
				...
				if (avatarList != null) {
					...
					for (Object pircBotObj : FozConfig.getManager().getBots().toArray()) {
						...
						if (bot != null)
							bot.setAvatar(URIUtil.encodeQuery("https://lilggamegenius.net/burgerpants/" + avatarFile.getName()));
					}
